...
const queueName = 'crm_qu';

const createChannel = async () => {
  try {
    const connection = await amqp.connect('amqps://mdvpqleh:omEC9S__1cfAnvhe0Qrh61qr26WvpWIH@puffin.rmq2.cloudamqp.com/mdvpqleh');
...
const consumeMessages = async (callback) => {
  const channel = await createChannel();
  try {
    console.log('start consume message')
